...
class SQLQuery {
    ...
    /** Insert Query **/
        function insertQuery($columns, $data) {
          $left = "`".implode('`,`', $columns)."`";
          $values = "'".implode("','", $this->sql($data))."'";
          mysql_query("INSERT INTO `".$this->_table."` ($left) VALUES ($values)") or die(mysql_error());
        }
	...
	function where($field, $value, $equation) {
		$this->_extraConditions .= '`'.$this->_model.'`.`'.$field.'` '.$equation.' \''.$this->sql($value).'\' AND ';
	}

	function like($field, $value) {
		$this->_extraConditions .= '`'.$this->_model.'`.`'.$field.'` LIKE \'%'.$this->sql($value).'%\' AND ';
	}
	...
	function search() {
		...
		if ($this->_hO == 1 && isset($this->hasOne)) {
			...
			foreach ($this->hasOne as $alias => $model) {
				...
				$singularAlias = ucfirst(strtolower($alias))."ID";
				$from .= 'LEFT JOIN `'.$table.'` as `'.$alias.'` ';
				$from .= 'ON `'.$this->_model.'`.`'.ucfirst($this->_model).'ID` = `'.$alias.'`.`'.ucfirst($this->_model).'ID`  ';
			}
		}
		
		if ($this->_hOi == 1 && isset($this->hasOne)) {
			
			foreach ($this->hasOne as $alias => $model) {
				$table = strtolower($inflect->pluralize($model));
				$singularAlias = ucfirst(strtolower($alias))."ID";
				$from .= 'LEFT JOIN `'.$table.'` as `'.$alias.'` ';
				$from .= 'ON `'.$this->_model.'`.`'.$singularAlias.'` = `'.$alias.'`.`'.$singularAlias.'`  ';
			}
		}

		if ($this->id) {
			$conditions .= '`'.$this->_model.'`.`'.ucfirst($this->_model).'ID` = \''.mysql_real_escape_string($this->id).'\' AND ';
		}
			...
			while ($row = mysql_fetch_row($this->_result)) {
				...
				if ($this->_hM == 1 && isset($this->hasMany)) {
					foreach ($this->hasMany as $aliasChild => $modelChild) {
						...
						$conditionsChild .= '`'.$aliasChild.'`.`'.ucfirst(strtolower($this->_model)).'ID` = \''.$tempResults[$this->_model][ucfirst(strtolower($this->_model)).'ID'].'\'';
	...
	function delete() {
		if ($this->id) {
			$query = 'DELETE FROM '.$this->_table.' WHERE `'.ucfirst(strtolower($this->_model)).'ID`=\''.mysql_real_escape_string($this->id).'\'';
			$this->_result = mysql_query($query, $this->_dbHandle);
			$this->clear();
			if ($this->_result == 0) {
			    /** Error Generation **/
	...
	function save() {
		...
		if (isset($this->id)) {
			...
			$updates = substr($updates,0,-1);
                        $query = 'UPDATE '.$this->_table.' SET '.$updates.' WHERE `'.ucfirst(strtolower($this->_model)).'ID`=\''.mysql_real_escape_string($this->id).'\'';
		} else {
			$fields = '';
			$values = '';
			foreach ($this->_describe as $field) {
		...
		if ($this->_result == 0) {
            /** Error Generation **/
            echo "$query". mysql_error($this->_result);
			return -1;
        }
	}
